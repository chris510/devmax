...
`feedback` is one to three sentences, and its content depends on accuracy:
  - If accuracy <= 2: state the correct essential account directly, in plain terms.
    Do not just note that it was wrong or incomplete. This is the single most important
...
Do not use an em dash in generated text. Use a colon, period, comma, or middle dot. \
...
Do not use an em dash in `feedback`. Use a colon, period, comma, or middle dot.
...
no multi-part sub-questions, no preamble. Do not use an em dash. Use a colon, \
period, comma, or middle dot when punctuation is needed.\
"""
...
of the work (`estimate_source: generated`). Set `estimate_confidence` honestly. Use \
...
Do not use an em dash in generated labels or prose. Use a colon, period, comma, or \
middle dot. Preserve punctuation exactly inside every `source_excerpt` copied from \
the guide.
...
not write multi-part checklists. Do not use an em dash in generated fields. Use a \
colon, period, comma, or middle dot. Preserve punctuation exactly in `source_excerpt`. \
Return only the structured fields. No preamble, \
...
inference, or unsupported addition. Do not use an em dash in `reason` or `repair`. \
Use a colon, period, comma, or middle dot. Preserve every exact `evidence_spans` \
substring byte-for-byte. Return only the structured fields. No preamble, \
...
_SUMMARY_WRAPPERS = "\"'`«»‘’“”「」『』"
_EM_DASH_SEPARATOR = re.compile(r" *\N{EM DASH} *")


def clean_generated_copy(text: str) -> str:
    """Remove an em dash introduced by a model from product-owned copy.

    This is intentionally called only on generated fields. Learner transcripts,
    trusted answer material, guide text, source excerpts, and grounding evidence
    remain verbatim so punctuation cleanup cannot change evidence or provenance.
    """
    return _EM_DASH_SEPARATOR.sub(": ", text).strip()


def _clean_generated_structure(
    value: Any, *, field: str = "", verbatim: bool = False
) -> Any:
    """Clean provider-authored strings while preserving copied evidence fields."""
    preserve = verbatim or field in {"source_excerpt", "evidence_spans"}
    if isinstance(value, str):
        return value if preserve else clean_generated_copy(value)
    if isinstance(value, list):
        return [
            _clean_generated_structure(item, field=field, verbatim=preserve)
            for item in value
        ]
    if isinstance(value, dict):
        return {
            key: _clean_generated_structure(item, field=key, verbatim=preserve)
...
def clean_summary(text: str) -> str:
    ...
    """
    cleaned = text.strip().strip(_SUMMARY_WRAPPERS).strip()
    return clean_generated_copy(cleaned)
...
class ReattemptResult:
    """Turn 3's grade. One axis and a summary, with no composite by design.
...
async def generate_question(
    *,
    ...
    question = clean_generated_copy(str(data.get("question", "")))
    if not question:
...
def parse_score_result(data: dict[str, Any], *, probes_used: int) -> ScoreResult:
    ...
    probe = clean_generated_copy(str(data.get("follow_up_question", "")))
    ...
    return ScoreResult(
        ...
        feedback=clean_generated_copy(str(data.get("feedback", ""))),
...
def parse_score_v2_result(data: dict[str, Any], *, probes_used: int) -> ScoreResult:
    ...
    probe = clean_generated_copy(data["follow_up_question"])
    ...
    feedback = clean_generated_copy(data["feedback"])
...
async def coach_answer(
    *,
    ...
    feedback = clean_generated_copy(str(data.get("coaching_feedback", "")))
    if not feedback:
...
async def evaluate_lesson_check(
    *,
    ...
    feedback = (
        clean_generated_copy(feedback_value)
...
async def extract_lesson(
    *,
    ...
    return [
        _clean_generated_structure(concept)
...
async def verify_lesson_grounding(
    *,
    ...
    return [
        _clean_generated_structure(finding)
...
async def import_guide(
    *,
    ...
    return _clean_generated_structure(raw)
...
async def propose_cards(
    *,
    ...
    return [
        _clean_generated_structure(candidate)
