...
def _debrief_eligible(graph: sp.PlanGraph, item: StudyPlanItem) -> bool:
...
async def _item_detail(db: AsyncSession, graph: sp.PlanGraph, item: StudyPlanItem) -> ItemDetail:
    ...
    return ItemDetail(
        ...
        practice_debrief_eligible=_debrief_eligible(graph, item),
...
async def save_practice_debrief_draft(
    ...
    if item is None:
        raise HTTPException(status_code=404, detail="item not found")
    if not _debrief_eligible(graph, item):
